...
# Read in data
approvals_short <- fda_approvals %>% select(Brand_Name, Therapeutic_Area, TA_subgroup, Asian, Black, Hispanic, White)
...
comparison_df_summary <- comparison_df %>% 
  group_by(TA_subgroup, Demographic, Comparison) %>% 
  summarise(Count = n())
